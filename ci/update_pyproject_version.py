...
def get_arg() -> Optional[str]:
    ...
    Raises:
        SystemExit: Invalid tag format([major].[minor].[patch])

    Returns:
        Optional[str]: [major].[minor].[patch] or None if not specified
    """
    if len(sys.argv) == 2:
        new_tag = sys.argv[1]
        ...
        error_message = (
            "Please enter ./update_pyproject_version.py [x.x.x] Exit the program."
        )
        sys.exit(error_message)
    else:
        new_tag = None
...
def create_ver(input_ver: Optional[str]) -> Tuple[bool, str, TOMLFile]:
    ...
    Args:
        input_ver (Optional[str]): [major].[minor].[patch]
    ...
    if input_ver is not None:
        ...
        if input_ver <= current_data:
            error_message = f"The specified tag '{input_ver}' must be greater than the current version '{current_data}'. Exit the program."
            sys.exit(error_message)
        return True, input_ver, toml
    else:
        # No update needed if no version is specified
        return False, current_data, toml
