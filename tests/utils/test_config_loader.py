...
from project_a.utils.config_loader import find_config_file, load_config
...
def mock_project_structure(tmp_path: Path) -> Path:
    ...
    utils_dir = project_root / "project_a" / "utils"
    utils_dir.mkdir(parents=True)
    (utils_dir / "config_loader.py").touch()

    config_dir = project_root / "project_a"
...
def mock_config_loader(monkeypatch: MonkeyPatch, mock_project_structure: Path) -> Path:
    ...
    def mock_resolve(*args: Any, **kwargs: Any) -> Path:
        return mock_project_structure / "project_a" / "utils" / "config_loader.py"
...
def test_find_config_file_success(mock_config_loader: Path) -> None:
    ...
    """
    expected_path = mock_config_loader / "project_a" / "config.toml"
...
def test_find_config_file_custom_name(mock_config_loader: Path) -> None:
    ...
    """
    custom_config = mock_config_loader / "project_a" / "custom_config.toml"
    custom_config.touch()
...
def test_load_config_file_not_found(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "project_a.utils.config_loader.find_config_file", mock_find_config_file
    )
    with pytest.raises(FileNotFoundError):
        load_config()
...
def test_load_config_invalid_toml(
    ...
    Then: 例外が発生する
    """
    invalid_config = mock_config_loader / "project_a" / "invalid_config.toml"
    invalid_config.write_text("invalid toml content")
    monkeypatch.setattr(
        "project_a.utils.config_loader.find_config_file",
...
def test_load_config_empty_file(
    ...
    Then: 空の辞書が返される
    """
    empty_config = mock_config_loader / "project_a" / "empty_config.toml"
